(() => {
  ...
  async function loadPublishedArticles() {
    ...
    if (error) { console.error("Gagal memuat tulisan:", error); return []; }
  ...
  async function renderArticleList() {
    ...
    if (!articles.length) {
      list.innerHTML = '<p class="article-loading">Belum ada tulisan yang diterbitkan.</p>';
      return;
    }
  ...
  async function loadArticleDetail() {
    ...
    if (!key) { body.innerHTML = "<p>Tulisan tidak ditemukan.</p>"; return; }
    ...
    if (result.error || !result.data) {
      title.textContent = "Tulisan tidak ditemukan";
      const desc = document.getElementById("articleDescription"); if (desc) desc.textContent = "Tulisan yang kamu cari tidak tersedia.";
      body.innerHTML = "<p>Tulisan tidak ditemukan atau belum diterbitkan.</p>";
      document.title = "Tulisan tidak ditemukan — Waskara";
      return;
    }
    ...
    body.innerHTML = article.content || "<p>Tulisan ini belum memiliki isi.</p>";
    document.title = `${article.title || "Tulisan"} — Waskara`;
  }
  ...
  async function renderSearch(query = "") {
    ...
    searchResults.innerHTML = matches.length
      ...
      : '<p style="color:var(--muted);font-size:11px;">Tulisan tidak ditemukan.</p>';
  }
